...
const NODE_GLOBALS = () => {
    ...
    const merged = mergeDescriptors(subtract(global, ecmaGlobals), ecmaGlobals, {
        require,
        __dirname: (fileName) => path.dirname(fileName),
...
})(exports.EnvironmentPresetKind || (exports.EnvironmentPresetKind = {}));
...
const BROWSER_GLOBALS = () => {
    const { window } = new jsdom.JSDOM("", { url: "https://example.com" });
    const ecmaGlobals = ECMA_GLOBALS();
...
function getPresetForLexicalEnvironment(env) {
    if (env.preset != null)
        ...
        return exports.EnvironmentPresetKind.NONE;
}
/**
 * Gets a value from a Lexical Environment
 * @param {Node?} node
    ...
    if (objectPath.has(env.env, firstBinding)) {
        ...
        switch (path) {
            ...
            case "__filename":
                ...
                return preset === exports.EnvironmentPresetKind.NODE && typeof literal === "function" && node != null
...
function setInLexicalEnvironment({ env, path, value, reporting, node, newBinding = false }) {
    ...
    if (objectPath.has(env.env, firstBinding) || newBinding || env.parentEnv == null) {
        ...
        if (objectPath.has(env.env, path) && objectPath.get(env.env, path) === value)
            return;
        // Otherwise, mutate it
...
function createLexicalEnvironment({ inputEnvironment: { extra, preset }, policy, getCurrentNode }) {
    let envInput;
    switch (preset) {
        case exports.EnvironmentPresetKind.NONE:
            envInput = mergeDescriptors(extra);
            break;
        case exports.EnvironmentPresetKind.ECMA:
            envInput = mergeDescriptors(ECMA_GLOBALS(), extra);
            break;
        case exports.EnvironmentPresetKind.NODE:
            envInput = mergeDescriptors(NODE_GLOBALS(), extra);
            break;
        case exports.EnvironmentPresetKind.BROWSER:
 ...
 */
function evaluateSimpleLiteral(node) {
    if (ts.isStringLiteralLike(node))
        return { success: true, value: node.text };
    else if (isBooleanLiteral(node))
        return { success: true, value: node.kind === ts.SyntaxKind.TrueKeyword };
    ...
    else if (ts.isIdentifier(node) && node.text === "Infinity")
        return { success: true, value: Infinity };
    else if (ts.isIdentifier(node) && node.text === "NaN")
        return { success: true, value: NaN };
    else if (ts.isIdentifier(node) && node.text === "null")
 ...
 */
function evaluateVariableDeclaration({ node, environment, evaluate, stack, statementTraversalStack }, initializer) {
    ...
    if (initializerResult == null && !ts.isIdentifier(node.name)) {
 ...
 */
function getDotPathFromNode(_a) {
    var { node, evaluate } = _a, rest = tslib.__rest(_a, ["node", "evaluate"]);
    if (ts.isIdentifier(node)) {
        ...
        return getDotPathFromNode(Object.assign({ node: node.expression, evaluate }, rest));
    }
    else if (ts.isTypeAssertion(node)) {
        return getDotPathFromNode(Object.assign({ node: node.expression, evaluate }, rest));
    }
    else if (ts.isPropertyAccessExpression(node)) {
        let leftHand = getDotPathFromNode(Object.assign({ node: node.expression, evaluate }, rest));
        if (leftHand == null)
            leftHand = (evaluate.expression(node.expression, rest.environment, rest.statementTraversalStack));
        let rightHand = getDotPathFromNode(Object.assign({ node: node.name, evaluate }, rest));
        if (rightHand == null)
            rightHand = (evaluate.expression(node.name, rest.environment, rest.statementTraversalStack));
        ...
        if (leftHand == null)
            leftHand = (evaluate.expression(node.expression, rest.environment, rest.statementTraversalStack));
        const rightHand = (evaluate.expression(node.argumentExpression, rest.environment, rest.statementTraversalStack));
        if (leftHand == null || rightHand == null)
            return undefined;
        return `${leftHand}.${rightHand}`;
    }
...
class UnexpectedNodeError extends EvaluationError {
    constructor({ node, message = `Unexpected Node: '${ts.SyntaxKind[node.kind]}'` }) {
        super({ message, node });
    }
}
 ...
 * @returns {Literal}
 */
function evaluateBinaryExpression(_a) {
    var { node, environment, evaluate, logger, statementTraversalStack, reporting } = _a, rest = tslib.__rest(_a, ["node", "environment", "evaluate", "logger", "statementTraversalStack", "reporting"]);
    ...
    const leftIdentifier = getDotPathFromNode(Object.assign({ node: node.left, environment, evaluate, logger, statementTraversalStack, reporting }, rest));
 ...
 * @return {boolean}
 */
function expressionContainsSuperKeyword(expression) {
    if (isSuperExpression(expression))
        return true;
        ...
        return expressionContainsSuperKeyword(expression.expression);
    else if (ts.isAsExpression(expression))
        return expressionContainsSuperKeyword(expression.expression);
    else if (ts.isTypeAssertion(expression))
        return expressionContainsSuperKeyword(expression.expression);
    else {
        return false;
    }
 ...
 */
function evaluateCallExpression({ node, environment, evaluate, statementTraversalStack, logger }) {
    const evaluatedArgs = [];
    ...
    if (isLazyCall(expressionResult)) {
        ...
        const value = expressionResult.invoke(currentThisBinding != null
            ? currentThisBinding.literal
            : undefined, ...evaluatedArgs);
 ...
 */
function evaluateParameterDeclarations({ node, evaluate, environment, statementTraversalStack }, boundArguments, context) {
    // 'this' is a special parameter which is removed from the emitted results
    const parameters = node.filter(param => !(ts.isIdentifier(param.name) && param.name.text === "this"));
    for (let i = 0; i < parameters.length; i++) {
        ...
        if (parameter.dotDotDotToken != null) {
            ...
            if (context != null && ts.isIdentifier(parameter.name) && (hasModifier(parameter, ts.SyntaxKind.PublicKeyword) ||
 ...
 * @returns {Promise<Literal>}
 */
function evaluateArrowFunctionExpression(_a) {
    var { node, environment, evaluate, stack, statementTraversalStack, reporting } = _a, rest = tslib.__rest(_a, ["node", "environment", "evaluate", "stack", "statementTraversalStack", "reporting"]);
    const arrowFunctionExpression = hasModifier(node, ts.SyntaxKind.AsyncKeyword)
        ? async (...args) => {
            ...
            evaluateParameterDeclarations(Object.assign({ node: node.parameters, environment: localLexicalEnvironment, evaluate,
                stack,
 ...
 * @returns {Promise<Literal>}
 */
function evaluateBooleanLiteral({ node }) {
 ...
 * @returns {Promise<Literal>}
 */
function evaluateArrayLiteralExpression({ node, environment, evaluate, statementTraversalStack }) {
    ...
    for (const element of node.elements) {
        ...
        if (ts.isSpreadElement(element) && isIterable(nextValue)) {
 ...
 * @return {boolean}
 */
function isVarDeclaration(declarationList) {
    return declarationList.flags !== ts.NodeFlags.Const && declarationList.flags !== ts.NodeFlags.Let;
}
 ...
 * @return {T?}
 */
function findNearestParentNodeOfKind(from, kind) {
    ...
    while (true) {
        ...
        if (currentParent.kind === kind) {
            const combinedNodeFlags = ts.getCombinedNodeFlags(currentParent);
            const isNamespace = (((combinedNodeFlags & ts.NodeFlags.Namespace) !== 0) ||
                ((combinedNodeFlags & ts.NodeFlags.NestedNamespace) !== 0));
            if (!isNamespace)
                return currentParent;
        }
        if (ts.isSourceFile(currentParent))
            return undefined;
    }
}
 ...
 */
function getDeclarationName({ node, evaluate, environment, statementTraversalStack }) {
...
function getImplementationForDeclarationWithinDeclarationFile(options) {
    ...
    const moduleDeclaration = ts.isModuleDeclaration(node) ? node : findNearestParentNodeOfKind(node, ts.SyntaxKind.ModuleDeclaration);
 ...
 * @returns {Promise<Literal>}
 */
function evaluateIdentifier(_a) {
    var { node, environment, typeChecker, evaluate, stack, logger, reporting, statementTraversalStack } = _a, rest = tslib.__rest(_a, ["node", "environment", "typeChecker", "evaluate", "stack", "logger", "reporting", "statementTraversalStack"]);
    ...
    if (valueDeclaration != null) {
        if (valueDeclaration.getSourceFile().isDeclarationFile) {
            const implementation = getImplementationForDeclarationWithinDeclarationFile(Object.assign({ node: valueDeclaration, statementTraversalStack, environment, evaluate, logger, reporting, typeChecker, stack }, rest));
        ...
        if (ts.isVariableDeclaration(valueDeclaration) && valueDeclaration.getSourceFile().fileName === node.getSourceFile().fileName && valueDeclaration.pos > node.pos) {
            // The 'var' keyword declares a variable that is defined, but which rvalue is still undefined
            if (ts.isVariableDeclarationList(valueDeclaration.parent) && isVarDeclaration(valueDeclaration.parent)) {
 ...
 */
function evaluateBlock({ node, environment, evaluate }) {
    ...
    for (let i = 0; i < node.statements.length; i++) {
        ...
        if (ts.isConstructorDeclaration(node.parent) && i === 0 && ts.isExpressionStatement(statement) && ts.isCallExpression(statement.expression) && isSuperExpression(statement.expression.expression))
            continue;
 ...
 * @returns {Promise<Literal>}
 */
function evaluatePrefixUnaryExpression({ node, environment, evaluate, reporting, statementTraversalStack }) {
 ...
 * @returns {Promise<Literal>}
 */
function evaluatePropertyAccessExpression({ node, environment, evaluate, statementTraversalStack }) {
    const expressionResult = (evaluate.expression(node.expression, environment, statementTraversalStack));
    ...
    if (typeof match === "function" && statementTraversalStack.includes(ts.SyntaxKind.CallExpression)) {
        return {
            [LAZY_CALL_FLAG]: 0 /* CALL */,
            invoke: (overriddenThis, ...args) => {
 ...
 * @returns {Promise<Literal>}
 */
function evaluateElementAccessExpression({ node, environment, evaluate, statementTraversalStack }) {
    const expressionResult = (evaluate.expression(node.expression, environment, statementTraversalStack));
...
function evaluateFunctionDeclaration(options) {
    const { node, environment, evaluate, stack, reporting } = options, rest = tslib.__rest(options, ["node", "environment", "evaluate", "stack", "reporting"]);
    const nameResult = node.name == null ? undefined : node.name.text;
    const _functionDeclaration = hasModifier(node, ts.SyntaxKind.AsyncKeyword)
 ...
 * @returns {Promise<Literal>}
 */
function evaluatePostfixUnaryExpression({ node, evaluate, environment, reporting, statementTraversalStack }) {
 ...
 */
class AsyncNotSupportedError extends EvaluationError {
    constructor({ message = `It is not possible to evaluate asynchronously: Optional dependency 'deasync' must be installed.'` }) {
        super({ message, node: ts.createEmptyStatement() });
    }
}
...
try {
    ...
    const importedModule = require("deasync");
 ...
 * @returns {Promise<void>}
 */
function evaluateForOfStatement({ node, environment, evaluate, logger, reporting, statementTraversalStack }) {
    // Compute the 'of' part
    const expressionResult = (evaluate.expression(node.expression, environment, statementTraversalStack));
 ...
 * @returns {Promise<void>}
 */
function evaluateForStatement({ node, environment, evaluate, reporting, statementTraversalStack }) {
    ...
    while (true) {
        ...
        const conditionResult = node.condition == null
            ? true
            : (evaluate.expression(node.condition, forEnvironment, statementTraversalStack));
 ...
 * @returns {Promise<void>}
 */
function evaluateWhileStatement({ node, environment, evaluate, logger, reporting, statementTraversalStack }) {
 ...
 * @returns {Promise<void>}
 */
function evaluateForInStatement({ node, environment, evaluate, logger, reporting, statementTraversalStack }) {
    // Compute the 'of' part
    const expressionResult = (evaluate.expression(node.expression, environment, statementTraversalStack));
    // Ensure that the initializer is a proper VariableDeclarationList
    if (!ts.isVariableDeclarationList(node.initializer)) {
 ...
 * @returns {Promise<Literal>}
 */
function evaluateFunctionExpression(_a) {
    var { node, environment, evaluate, stack, reporting } = _a, rest = tslib.__rest(_a, ["node", "environment", "evaluate", "stack", "reporting"]);
    const nameResult = node.name == null ? undefined : node.name.text;
    const _functionExpression = hasModifier(node, ts.SyntaxKind.AsyncKeyword)
 ...
 * @param {IEvaluatorOptions<FunctionDeclaration>} options
 */
function evaluateClassDeclaration({ node, environment, evaluate, stack, logger, reporting, statementTraversalStack }) {
    let extendedType;
    const ctorMember = node.members.find(ts.isConstructorDeclaration);
    const otherMembers = node.members.filter(member => !ts.isConstructorDeclaration(member));
    let ctor;
    if (ctorMember != null) {
    ...
    for (const member of otherMembers) {
        evaluate.nodeWithArgument(member, environment, hasModifier(member, ts.SyntaxKind.StaticKeyword)
            ? classDeclaration
            : classDeclaration.prototype, statementTraversalStack);
    }
 ...
 * @returns {Promise<void>}
 */
function evaluateConstructorDeclaration(_a) {
    var { node, environment, evaluate, stack, reporting } = _a, rest = tslib.__rest(_a, ["node", "environment", "evaluate", "stack", "reporting"]);
    /**
 ...
 * @returns {Literal}
 */
function evaluateClassExpression({ node, environment, evaluate, stack, logger, reporting, statementTraversalStack }) {
    let extendedType;
    const ctorMember = node.members.find(ts.isConstructorDeclaration);
    const otherMembers = node.members.filter(member => !ts.isConstructorDeclaration(member));
    let ctor;
    ...
    for (const member of otherMembers) {
        evaluate.nodeWithArgument(member, environment, hasModifier(member, ts.SyntaxKind.StaticKeyword)
            ? classExpression
            : classExpression.prototype, statementTraversalStack);
    }
 ...
 * @returns {Promise<Literal>}
 */
function evaluateAwaitExpression({ node, environment, evaluate, policy, statementTraversalStack }) {
    ...
    const result = syncAwait(evaluate.expression(node.expression, environment, statementTraversalStack));
 ...
 * @return {boolean}
 */
function inStaticContext(node) {
    let currentNode = node;
    while (currentNode != null && !ts.isSourceFile(currentNode)) {
        if (hasModifier(currentNode, ts.SyntaxKind.StaticKeyword))
            return true;
 ...
 */
function evaluateMethodDeclaration(_a, parent) {
    var { node, environment, evaluate, stack, statementTraversalStack, reporting } = _a, rest = tslib.__rest(_a, ["node", "environment", "evaluate", "stack", "statementTraversalStack", "reporting"]);
    ...
    const _methodDeclaration = hasModifier(node, ts.SyntaxKind.AsyncKeyword)
    ...
    if (node.parameters != null) {
        // 'this' is a special parameter which is removed from the emitted results
        const parameters = node.parameters.filter(param => !(ts.isIdentifier(param.name) && param.name.text === "this"));
        for (let i = 0; i < parameters.length; i++) {
            const parameter = parameters[i];
 ...
 * @returns {Promise<void>}
 */
function evaluatePropertyDeclaration({ environment, node, evaluate, statementTraversalStack, stack }, parent) {
 ...
 * @param {IndexLiteral} [parent]
 */
function evaluateGetAccessorDeclaration({ node, environment, evaluate, stack, reporting, statementTraversalStack }, parent) {
    const nameResult = (evaluate.nodeWithValue(node.name, environment, statementTraversalStack));
    const isStatic = inStaticContext(node);
    if (parent == null) {
        let updatedParent;
 ...
 * @returns {Promise<void>}
 */
function evaluateBindingName({ node, environment, evaluate, statementTraversalStack, reporting, logger }, rightHandValue) {
 ...
 * @param {IndexLiteral} parent
 */
function evaluateSetAccessorDeclaration(_a, parent) {
    var { node, environment, evaluate, statementTraversalStack, reporting, stack } = _a, rest = tslib.__rest(_a, ["node", "environment", "evaluate", "statementTraversalStack", "reporting", "stack"]);
    const nameResult = (evaluate.nodeWithValue(node.name, environment, statementTraversalStack));
    const isStatic = inStaticContext(node);
    /**
 ...
 * @returns {Promise<void>}
 */
function evaluateBindingElement({ environment, node, evaluate, logger, reporting, statementTraversalStack }, rightHandValue) {
    ...
    if (ts.isIdentifier(node.name) && node.propertyName != null) {
        ...
        const propertyValue = ts.isArrayBindingPattern(node.parent)
            ? rightHandValue
            : rightHandValue[propertyNameResult];
    ...
    else if (ts.isIdentifier(node.name) && node.propertyName == null) {
        // Compute the literal value of the name of the node
        const nameResult = node.name.text;
        // Extract the property value from the initializer. If it is an ArrayBindingPattern, the rightHandValue will be assigned as-is to the identifier
        const propertyValue = ts.isArrayBindingPattern(node.parent)
            ? rightHandValue
            : rightHandValue[nameResult];
        // Fall back to using the initializer of the BindingElement if the property value is null-like and if it has one
        const propertyValueWithInitializerFallback = propertyValue != null
            ? propertyValue
            : bindingElementInitializer;
...
function evaluateDecorator({ node, environment, evaluate, stack, statementTraversalStack }, [parent, propertyName, index]) {
    ...
    stack.push(tslib.__decorate([
        index != null
            ? tslib.__param(index, decoratorImplementation)
...
function evaluateNodeWithArgument(options, arg) {
    ...
    const { node } = options, rest = tslib.__rest(options, ["node"]);
 ...
 * @returns {Promise<Literal>}
 */
function evaluatePropertyName({ environment, node, evaluate, statementTraversalStack }) {
 ...
 */
function createNodeEvaluator({ typeChecker, policy, logger, stack, reporting, nextNode }) {
    let ops = 0;
    ...
    const nodeEvaluator = {
        expression: (node, environment, statementTraversalStack) => {
 ...
 * @returns {string}
 */
function stringifySyntaxKind(kind) {
...
class Logger {
     ...
     */
    logStatementTraversalStack(stack) {
        if (this.logLevel < 30 /* DEBUG */)
            return;
...
function isExpression(node) {
    return ts.isExpressionNode(node) || ts.isIdentifier(node);
}
...
function isStatement(node) {
    return ts.isStatementButNotDeclaration(node);
}
 ...
 */
function evaluate({ typeChecker, node, environment: { preset = exports.EnvironmentPresetKind.NODE, extra = {} } = {}, logLevel = 0 /* SILENT */, policy: { deterministic = false, network = false, console = false, maxOps = Infinity, maxOpDuration = Infinity, io = {
    read: true,
    write: false
}, process = {
    exit: false,
...
exports.AsyncNotSupportedError = AsyncNotSupportedError;
